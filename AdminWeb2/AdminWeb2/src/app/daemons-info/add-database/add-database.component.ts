...
export class AddDatabaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get<Databases[]>('http://localhost:63324/api/admin/daemons/databases/'+this.DaemonId)
    .subscribe  (data => {
  ...
  AddDatabase(SrvName:string,DbName:string,Login:string,Password:string){
    ...
    this.http.post<boolean>('http://localhost:63324/api/admin/daemons/AddDatabase',JSON.stringify(data),head)
    .subscribe(Response=>{
