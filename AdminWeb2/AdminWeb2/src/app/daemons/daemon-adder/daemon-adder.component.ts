...
export class DaemonAdderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.post<number>("http://localhost:63324/api/admin/daemons/GetId",JSON.stringify(data),head)
    .subscribe(Response =>{
      this.ID = Response
    })
  }
  ...
  AddDaemon(Nickname:string,DaemonTo:string,Description:string,){
    ...
    this.http.post("http://localhost:63324/api/admin/daemons/AddDaemon",JSON.stringify(data),head)
    .subscribe(Response => {
